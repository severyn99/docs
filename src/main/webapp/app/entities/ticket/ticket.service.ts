...
export class TicketService {
  ...
  protected convertDateFromClient(ticket: ITicket): ITicket {
    const copy: ITicket = Object.assign({}, ticket, {
      purchased: ticket.purchased && ticket.purchased.isValid() ? ticket.purchased.toJSON() : undefined
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((ticket: ITicket) => {
        ticket.purchased = ticket.purchased ? moment(ticket.purchased) : undefined;
      });
    }
    return res;
  }
